...
## 2026-06-26 - Optimize high-frequency date math with shared Date helpers

**Learning:** Object-heavy date abstractions in simple `YYYY-MM-DD` hot paths can add avoidable allocation overhead when iterating through an athlete's entire workout history for calendar rendering.
**Action:** Use the shared `datetime.ts` helpers for hot-path day math. They use native `Date` arithmetic and strict string parsing/slicing for the `YYYY-MM-DD` format. Use `setUTCFullYear` instead of `Date.UTC` to avoid the 2-digit year quirk (years 0–99 map to 1900–1999). Use `Math.floor((t2 - t1) / 86400000)` for safe timezone-free differences.

## 2026-06-28 - Use TypedArrays for numerical time-series algorithms
...
**Action:** Consolidate parallel `.map()` calls into a single-pass loop that pre-allocates each output series with `new Array(n)` and fills all of them in a single traversal.

## 2024-07-26 - Single-pass loops inside Svelte `$derived.by()` reactive blocks
...
## 2026-07-04 - Replace standard Array pre-allocation with Float64Array for performance

**Learning:** `uPlot` chart data is very sensitive to Array allocations. Creating numerical sequential arrays using `Array.from<number>({ length: n })` for `uPlot.AlignedData` incurs Boxing overhead and higher GC pause time on updates inside Svelte `$derived.by` blocks when handling large sequences (like stroke or workout history). `uPlot`'s `AlignedData` type already supports `TypedArray`, so using `new Float64Array(n)` drastically avoids Object instantiation overhead while retaining matching lengths.
**Action:** Always prefer pre-allocating Typed Arrays (like `new Float64Array(length)`) instead of standard Arrays (`Array.from` or `new Array`) when performing numerical sequential parsing and storing large metrics like `pace`, `spm`, `watts`, `t` that map to charts in high-frequency reactive blocks. When mixed types are expected like `(number | null)[]`, it is acceptable to stick to standard array casting or `any[]` if typescript strictness complains about `AlignedData` type mismatch, but we should use `Float64Array` wherever possible for continuous number streams.
