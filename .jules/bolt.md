...
**Action:** Inside `$derived.by()` blocks, consolidate parallel `.map()` calls into a single-pass loop that pre-allocates each output series with `new Array(n)` and fills all of them in a single traversal. Similarly, consolidate multiple `.reduce()` operations over the same dataset into one loop that directly accumulates values.
