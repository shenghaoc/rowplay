...
import { detailCacheTtlMs, isDetailCacheFresh, type DetailCacheEnv } from "./detailCache";
...
export async function queryWorkouts(
  ...
  const sortExpr: Record<WorkoutListQuery["sort"], string> = {
    ...
    pace: paceExpr,
    power:
      "CASE WHEN time > 0 AND watt_minutes IS NOT NULL THEN watt_minutes * 60.0 / time ELSE 0 END",
  };
...
export async function deleteLeaderboardEntry(
  ...
  try {
    await db
      .prepare("DELETE FROM leaderboard_entry WHERE user_id = ? AND sport = ? AND distance = ?")
...
export async function getLeaderboardEntries(db: D1Database | undefined): Promise<LeaderboardRow[]> {
  if (!db) return [];
  try {
    const res = await db
      .prepare(
