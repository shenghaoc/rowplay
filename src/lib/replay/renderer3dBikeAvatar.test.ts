...
describe("makeBikeAvatar", () => {
  it("builds a populated BikeErg rig at every quality tier", () => {
    for (const quality of ["medium"] as const) {
  ...
  it("keeps live and ghost rigs independent", () => {
    ...
    const pose = fallbackStrokePose("bike", 0.25 * Math.PI * 2);
    live.animate(0.25, false, pose, 10);
    ghost.animate(0.75, false, fallbackStrokePose("bike", 0.75 * Math.PI * 2), 90);
    live.animate(0.25, false, pose, 10);
