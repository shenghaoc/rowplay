 ...
 *
 * This module authors the reviewed product asset: one continuous skinned
 * surface, a stable generic skeleton, explicit contact-effectors, and
 * deterministic canonical clips for RowErg, SkiErg and BikeErg. No downloaded
 * mesh, scan, likeness, avatar generator, texture, user image, or athlete
 * telemetry contributes to it.
 *
...
function createSportClip(
  ...
  clip.userData = {
    ...
    role: "canonical generic technique base; analytic contacts override before render",
  };
...
function createRowCycleClip(): THREE.AnimationClip {
  const times = [0, 0.08, 0.18, 0.3, 0.38, 0.54, 0.72, 0.88, 1] as const;
  const leftClavicle = [
    [0.06, -0.03, -0.12],
    [0.07, -0.035, -0.13],
    [0.05, -0.025, -0.11],
...
function createSkiCycleClip(): THREE.AnimationClip {
  const times = [0, 0.08, 0.12, 0.24, 0.34, 0.48, 0.58, 0.78, 1] as const;
  const leftArm = [
    [-0.92, -0.22, -0.42],
    [-1.02, -0.23, -0.44],
    [-0.98, -0.21, -0.42],
