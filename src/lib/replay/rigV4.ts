...
function createBikeCycleClip(): THREE.AnimationClip {
  ...
  return createSportClip(
    ...
    ],
    {
      ...
      // Lean deepened (-0.28 -> -0.30 lumbar, -0.12 -> -0.135 thoracic,
      // ~3.3 deg total after the 1.6 pitch scale): the shoulders sat ~26 mm
      // beyond full arm reach to the hoods, so the palms hovered ~15 mm above
      // the grips with locked-straight elbows. A road rider's weight rests on
      // the hands; the deeper hinge closes the palm onto the hood and leaves
      // the elbows a soft weight-bearing bend instead of a locked reach.
      v4Spine: scalePitch(
        flipPitch([
          [-0.3, -0.018, 0],
          [-0.29, -0.012, 0],
