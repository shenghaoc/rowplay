	...
	while (pos < end) {
		...
		if (header & 0x40) {
			...
			for (let i = 0; i < numFields; i++) {
				fields.push({ num: dv.getUint8(pos), size: dv.getUint8(pos + 1), baseType: dv.getUint8(pos + 2) });
				pos += 3;
			}
			if (header & 0x20) {
...
function assignRecordField(rec: FitRecord, num: number, v: number) {
	switch (num) {
		case 253: rec.ts = v; break; // timestamp (s since 1989)
		case 5: rec.dist = v; break; // distance, scale 100 -> m
		case 6: rec.speed = v; break; // speed, scale 1000 -> m/s
		case 73: if (rec.speed == null) rec.speed = v; break; // enhanced_speed
		case 7: rec.power = v; break; // watts
		case 4: rec.cad = v; break; // cadence / stroke rate
		case 3: rec.hr = v; break; // heart rate
	}
}
...
function readBase(dv: DataView, pos: number, baseType: number, le: boolean): number | undefined {
	switch (baseType & 0x0f) {
		case 1: { const v = dv.getInt8(pos); return v === 0x7f ? undefined : v; }
		case 0: case 2: case 10: case 13: { const v = dv.getUint8(pos); return v === 0xff ? undefined : v; }
		case 3: { const v = dv.getInt16(pos, le); return v === 0x7fff ? undefined : v; }
		case 4: case 11: { const v = dv.getUint16(pos, le); return v === 0xffff ? undefined : v; }
		case 5: { const v = dv.getInt32(pos, le); return v === 0x7fffffff ? undefined : v; }
		case 6: case 12: { const v = dv.getUint32(pos, le) >>> 0; return v === 0xffffffff ? undefined : v; }
		case 8: { const v = dv.getFloat32(pos, le); return isNaN(v) ? undefined : v; }
		case 9: { const v = dv.getFloat64(pos, le); return isNaN(v) ? undefined : v; }
		default: return undefined; // strings / 64-bit ints: not needed here
	}
}
