...
export class CourseRenderer3D implements ReplayRenderer {
	...
	resize(cssWidth: number, cssHeight: number): void {
		...
		const cap =
			this.governor.level >= 2 ? 1 : this.governor.level === 1 ? Math.min(this.cfg.dprCap, 1.5) : this.cfg.dprCap;
	...
	render(state: RenderState, playing: boolean, themeName: 'light' | 'dark' = 'light'): void {
		...
		const rawDtMs = nowMs - this.lastNowMs;
		const dt = playing && Number.isFinite(this.lastNowMs) ? clampDt(rawDtMs) : 0;
		...
		if (this.reduceMotion) {
			...
			this.fovCurrent += (fovTarget - this.fovCurrent) * (this.cameraInit ? dampFactor(2.5, dt) : 1);
		}
		...
		} else if (this.camera.position.distanceToSquared(this.chase) > 9) {
