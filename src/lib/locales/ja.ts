export const ja = {
  ...
  landing: {
    ...
    feat3Title: "どこでも高速",
    feat3Body: "Cloudflare のグローバルエッジネットワークから配信 — 待ち時間ゼロの即時リプレイ。",
    tourEyebrow: "初回",
    tourTitle: "最初に試す 4 つ",
    tourBody:
...
アニメーションはワークアウトの実際のレートに同期します。記録された 1 ストロークごとに 1 漕ぎ（ポールプラントやペダル 1 回転）が再現され、キャッチのたびに水しぶきや雪煙が上がり、再生速度に合わせて速くなります。3D では、アスリートは胴体・頭・肩・腕・脚・手足を分けた人間スケールのモデルと競技別のウェアで描かれるため、玩具のマーカーではなくエルゴ上の選手として姿勢が読み取れます。手足は競技ごとの器具に沿って配置され、RowErg のハンドルとフットプレート、SkiErg のグリップとブーツ、BikeErg のハンドルバーとペダルから浮かないように見えます。コース面も競技ごとに変わり、RowErg は重なった水上レーン、SkiErg は圧雪の溝、BikeErg は縁石・レーンマーク・スピードバー付きのアスファルト/ベロドローム風トラックになります。チェイスカメラは身体の姿勢が見える距離を保ち、艇速が上がるとわずかに画角を広げます。
...
いいえ — ダッシュボードとリプレイはデフォルトで非公開です。必要に応じてリーダーボードに結果を公開したり、リプレイリンクを共有したりできますが、あなたが選択しない限り何も公開されません。
...
まだ困っていますか？ [FAQ](/docs/faq) がさらにカバーしています。このガイドのすべてのページはヘッダーの**ヘルプ**から開けます。`,
  ...
  replay: {
    ...
    noSamples: "そのファイルに使えるワークアウトサンプルがありません。別のファイルを試すか、フォーマットを確認してください。",
    fileReadError: "そのファイルを読み取れませんでした。CSV、TCX、FIT エクスポートかどうか確認してください。",
    importFailed: "そのファイルをインポートできませんでした。有効な CSV、TCX、FIT エクスポートであることを確認してください。",
    zone1: "Z1 リカバリー",
    zone2: "Z2 エンデュランス",
    zone3: "Z3 テンポ",
    zone4: "Z4 しきい値",
