...
2D ビューは引き続き軽量なプロシージャル選手モデルを描画します。3D ビューでは、rowplay 専用に制作されリポジトリが所有する、選手と競技器具のコンパクトな low-poly ジオメトリパックを読み込み、同じ接点駆動リグにその造形済みシェルを適合できます。動きとタイミングは記録サイクルに基づく決定的なイラスト表現のままで、手足は器具に揃い続けます。ローカルアセットを読み込めない場合もプロシージャル 3D モデルを利用でき、2D は安定した最終フォールバックです。モデルは汎用表現であり、スキャン、アバター生成器の出力、ユーザー画像、選手の肖像は含みません。
