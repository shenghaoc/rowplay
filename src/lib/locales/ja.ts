...
まだ困っていますか？ [FAQ](/docs/faq) がさらにカバーしています。このガイドのすべてのページはヘッダーの**ヘルプ**から開けます。`,
  ...
  settings: {
    ...
    dataTitle: "保存している内容",
    dataNote:
      "rowplay は Concept2 API 経由でワークアウトを必要に応じて読み取り、リアルタイムに取得してリプレイを即座に表示します。API トークンは SESSION_SECRET で httpOnly rp_tok Cookie に封印されます。セッションは暗号化された Cookie に保存され、サーバー側のデータベースは使用しません。切断またはデータ削除でセッションが消去されます。",
  ...
  token: {
    ...
    trustCacheBody:
      "ワークアウトデータは Concept2 API からリアルタイムに取得されます。公開共有やリーダーボード項目は公開した場合だけ作成されます。",
    apiToken: "API トークン",
