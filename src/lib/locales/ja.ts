...
3D では**画質**セレクターで低・中・高・Ultra を選べます。WebGPU 対応端末では、よりリッチな Ultra パスを先に試し、WebGL がフォールバックになります。端末が滑らかなフレームレートを維持できない場合は、解像度、次にエフェクトの順で自動的に下げます。リプレイのアニメーションは OS の「視差効果を減らす」設定に従います。
