export const ja = {
	...
	docs: {
		...
		openDashboard: 'ダッシュボードを開く',
		openSource: 'ソースを開く',
		guideMarkdown: `# rowplay ユーザーガイド

rowplay は Concept2 ログブックの結果を、RowErg、SkiErg、BikeErg 向けのダッシュボード、リプレイ、比較、リーダーボード体験に変換します。

## デモモードから始める

デモモードが既定です。サインインせずに rowplay を開くと決定的なサンプルワークアウトが読み込まれるため、Concept2 アカウントなしでダッシュボードとリプレイを試せます。

1. /dashboard を開きます。
2. リストからワークアウトを選びます。
3. **Replay** を押し、再生、一時停止、タイムライン操作、速度変更を使います。
4. /leaderboard を開き、サンプルデータでゴーストレースを試します。

## Concept2 ログブックを接続する

本番の認証は bring-your-own-token 方式です。個人の Concept2 API token は HTTPS で一度だけ送信され、Worker で検証され、httpOnly rp_tok cookie に封印されます。KV はセッション ID を保存し、D1 はキャッシュされたワークアウトとリプレイデータを保存します。token は KV や D1 には保存されません。

1. Concept2 ログブックで **Edit Profile -> Applications** を開きます。
2. 個人 API token をコピーします。
3. rowplay で /auth/token を開きます。
4. token を貼り付けて送信します。
5. /dashboard を開き、**同期** でログブック全履歴を読み込みます。

/settings で切断したり、キャッシュされたアカウントデータを削除したりできます。

## ダッシュボードを読む

- 種目と距離のフィルターでワークアウト一覧を絞り込みます。
- 合計、ペース傾向、PB、年間目標、トレーニング負荷を確認します。
- 最新ワークアウトを直接開くか、一覧から 2 つの結果を比較します。
- タグとフィルターを使い、特定のセッションを後で探しやすくします。

rowplay に最近の履歴しかない場合、長期の PB や傾向を信頼する前に **同期** を実行してください。

## ワークアウトをリプレイする

- 再生または一時停止でリプレイを操作します。
- タイムラインを動かして特定の地点を確認します。
- 速度を 0.5x から 8x まで変更します。
- ブラウザが WebGL をサポートする場合、2D と 3D レンダラーを切り替えます。
- ワークアウト中の地点にコーチングメモを追加します。
- CSV、JSON、またはリプレイデータが必要なときにエクスポートします。

Concept2 が提供する場合はストローク単位のデータを使います。ストロークデータがないワークアウトは split ベースのリプレイに戻るため、コースは引き続き再生されます。

## ゴーストと競い、ワークアウトを比較する

- /leaderboard で標準距離の結果を探し、ライバルのゴーストを開始します。
- リプレイ操作で自分のペースをゴーストと比較します。
- /compare で 2 つのワークアウトを横並びに要約します。
- 他の人にワークアウトを見てもらうときは公開リプレイリンクを共有します。

リーダーボードへの公開は任意で、取り消し可能です。元の Concept2 ログブック項目は変更されません。

## ライブモードとインポート

ライブモードはセッション後に新しいワークアウトをポーリングし、新しいデータが現れたら通知できます。心拍インポートは、ログブック項目に心拍が含まれていない場合に外部 HR データを統合できます。

ローカルの認証、同期、ライブモード、KV/D1 テストには npm run preview を使います。通常の npm run dev は UI 作業には高速ですが、Workers ランタイムではなく、本番 KV/D1 バインディングも提供しません。

## コントリビューター向け文書ポリシー

リポジトリ向けの英語ガイドは docs/usage.md にあります。アプリ内サイトは locale 辞書からガイド内容を表示するため、すべての同梱言語が同じ i18n 経路を使います。

ユーザーに見える挙動、ワークフロー、ルート、認証、データ処理、セットアップ、デプロイ期待値が変わる場合は、同じ pull request で docs/usage.md、README.md、または関連するリポジトリ文書を更新してください。

ローカルでは npm run check:docs -- origin/main でゲートを実行します。`
	},
	dashboard: {
		eyebrow: 'あなたのログブック',
		title: '結果とリプレイ',
