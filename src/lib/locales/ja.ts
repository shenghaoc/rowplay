export const ja = {
	...
	replay: {
		...
		raceVerdictWinSession:
			'{date} の {distance} に {seconds} 秒（+{m} m）で勝利',
		raceVerdictLoseSession:
			'{date} の {distance} に {seconds} 秒（−{m} m）で敗北',
		raceVerdictWinPace: '{pace} ペースボートに {seconds} 秒（+{m} m）で勝利',
		raceVerdictLosePace: '{pace} ペースボートに {seconds} 秒（−{m} m）で敗北',
		raceVerdictWinFile: '{name} に {seconds} 秒（+{m} m）で勝利',
		raceVerdictLoseFile: '{name} に {seconds} 秒（−{m} m）で敗北',
		raceFinished: 'レース終了',
		play: '再生',
		pause: '一時停止',
		viewToggle: 'コース表示',
