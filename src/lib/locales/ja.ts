...
どちらのビューも、ダウンロードやスキャンしたアバターではなく、軽量なプロシージャル選手モデルを描画します。上腕・前腕と大腿・下腿は一定の比率を保ち、明確な肘と膝で曲がります。形のある胴体、骨盤、関節、ウェア、手前側と奥側のコントラストにより、リプレイの大きさでも各ポーズを読み取れます。手足はサイクル全体を通して対応する器具に揃います。このモデルは汎用的なイラストであり、選手の体格、服装、外見を再現するものではありません。
