export const ja = {
  ...
  landing: {
    ...
    feat3Title: "どこでも高速",
    feat3Body:
      "Cloudflare からグローバル配信し、Concept2 のライブデータでワークアウト後のリプレイを表示。",
    tourEyebrow: "初回",
    tourTitle: "最初に試す 4 つ",
    tourBody:
      "ダッシュボードを見て、リプレイを開き、過去の自分の記録と競い、外部で確認したいデータをエクスポートします。",
    tourDashboard: "ダッシュボード：合計、トレンド、PB",
    tourReplay: "リプレイ：同期したコースとゲージ",
    tourGhost: "ゴーストレース：過去の記録または目標ペースを追う",
    tourExport: "エクスポート：CSV、JSON、TCX",
  ...
  docs: {
    ...
    contextual: {
      ...
      workflows: "リプレイ、ゴースト、エクスポートの仕組みを知る",
    },
    sections: {
      overview: {
        navTitle: "概要",
...
rowplay は、インドアのローイング・スキー・バイクのワークアウトを探索できるものに変えます。合計とトレンドのダッシュボード、ストロークごとのリプレイです。
...
- [はじめに](/docs/getting-started) — デモモードとログブックの接続。
...
- [よくある操作](/docs/workflows) — リプレイ、過去セッションのゴースト、エクスポート。
...
4. トークンを貼り付けて送信する。
5. ダッシュボードを開く。rowplay は Concept2 API から全履歴を直接取得します。

トークンは暗号化された接続で一度だけ送られ、保護されたブラウザの Cookie のみに保持されます。rowplay のサーバーはワークアウトやトークンを保存しません。

## 接続を解除する

接続を解除するにはヘッダーの**ログアウト**を使います。[データ](/settings)にはエクスポートとホームタイムゾーンが残ります。Concept2 ログブックが変更されることはありません。`,
      },
      rowingMetrics: {
        navTitle: "ローイングの基礎",
        markdown: `# ローイングの基礎
...
アニメーションはワークアウトの有効な記録サイクルに同期します。各ストローク行で、演出された 1 漕ぎ・ポール 1 サイクル・ペダル 1 回転が進み、時間も距離も進まないインターバルの基準行は無視されます。見える動きは決定的なイラスト表現であり、計測されたバイオメカニクスではありません。Concept2 はフォースカーブ、ハンドル軌道、ドライブ長、関節位置、姿勢を提供せず、rowplay はペースからワットを算出します。RowErg は脚、体幹、腕の順に動き、リカバリーでは逆順になります。SkiErg はリーチ、プラント/プル、リカバリーを分け、BikeErg はケイデンスで動くクランクと距離で回る車輪を分離します。2D と 3D の両方で、水・雪・トラックが競技別に描かれます。3D では手足が器具の接点から離れず、より近い競技別チェイスカメラが位置と注視点の両方を滑らかに追従します。

2D ビューは引き続き軽量なプロシージャル選手モデルを描画します。3D では、rowplay のリポジトリが所有するコンパクトな汎用選手を、単一のスキンメッシュとして読み込みます。RowErg、SkiErg、BikeErg にはそれぞれ制作済みの標準テクニッククリップがあり、リプレイのポーズと時間から決定的にサンプリングされます。その後、解析的な手足の補正によって、基準となる器具との接点を維持します。V4 を読み込めない場合は V3 の造形済みジオメトリとプロシージャル 3D を利用でき、Canvas 2D が安定した最終フォールバックになります。モデルとクリップは汎用の演出であり、選手固有のバイオメカニクスではありません。アセットには人物スキャン、アバター生成器の出力、ユーザー画像、記録済みモーション、選手の肖像は含まれません。

3D の RowErg 表現では、艇首を進行方向に向けた全長約 7.8 m のオープンコックピット・シングルスカルを使用します。選手は艇尾を向き、目に見えるストラップとヒール拘束で、艇尾側に固定された 42° のストレッチャーへ足を固定します。固定オールロックにより、左右のハンドルはキャッチから下胸部まで導かれます。

3D では**画質**セレクターで低・中・高・Ultra を選べます。Ultra は WebGPU が必要で、WebGL だけの端末では高にとどまります。端末が滑らかなフレームレートを維持できない場合は、解像度、次にエフェクトの順で自動的に下げます。リプレイのアニメーションは OS の「視差効果を減らす」設定に従います。
...
1. 自分のワークアウトをリプレイで開く。
2. ゴースト操作で比較できる過去セッションを選ぶ。
3. 過去の記録が追いかける 2 艇目として現れます。

自分の過去の記録ともレースできます。自己ベスト挑戦がどこで時間を稼ぎ、どこで失ったかが正確に分かります。

## エクスポート

[データ](/settings)ページでは、ライブのログブックを CSV または JSON でダウンロードでき、ストロークデータのあるワークアウトは TCX でもエクスポートできます。

## データを最新に保つ

ダッシュボードとリプレイのデータは Concept2 からライブで取得されます。**ライブモード**はログブックを確認して新しいワークアウトを知らせることもできます。`,
      },
      faq: {
        navTitle: "FAQ",
        markdown: `# FAQ
...
いいえ。ダッシュボードとリプレイは非公開で、rowplay には公開共有やリーダーボード機能はありません。

## rowplay は Concept2 ログブックを変更する？

決して変更しません。rowplay は読み取り専用で、元のログブックエントリーを変えることはありません。
...
ダッシュボードを再読み込みして最新の Concept2 履歴を取得し、ワークアウトが Concept2 ログブックにあるか確認してください。
...
ログブックに心拍があるのは、ワークアウト中にベルトや時計が接続されていた場合だけです。元のワークアウトに Concept2 上の心拍が含まれるか確認してください。
...
まずワークアウトが Concept2 ログブックに届いているか確認してください（マシンか ErgData アプリからのアップロードが必要です）。次にダッシュボードを再読み込みするか、ライブモードを有効にして自動確認させてください。
...
まだ困っていますか？ [FAQ](/docs/faq) がさらにカバーしています。このガイドのすべてのページはヘッダーの**ヘルプ**から開けます。`,
  ...
  dashboard: {
    ...
    recentNote:
      "最近のワークアウトを表示中 — 全履歴を読み込んで正確な PB とトレンドを確認してください。",
    latest: "最新",
    distance: "距離",
    time: "時間",
    avgRate: "平均レート",
    ...
    sectionWorkoutsBody: "深い分析パネルに入る前に、ワークアウトを絞り込み、再生できます。",
  ...
  workoutList: {
    ...
    windowed: "{n} 件のワークアウト · パフォーマンス向上のため仮想化",
  ...
  replay: {
    ...
    moments: {
      title: "トレーニングハイライト",
      subtitle: "このリプレイで最初に確認すべきポイント。",
      lowResolution: "スプリットベース",
      jump: "シーンへ移動",
      bpm: "bpm",
      "best-sustained": "ベスト持続区間",
      "slower-patch": "遅い区間",
      "efficient-rhythm": "最も効率的なリズム",
      "finish-trend": "フィニッシュ傾向",
      "best-rep": "ベストレップ",
      "slowest-rep": "スローレップ",
      reasonBestSustained: "本日の基準より {delta}% 速い。",
      reasonSlowerPatch: "本日の基準より {delta}% 遅い。リズムと回復を確認。",
      reasonEfficientRhythm: "最高のストロークレートなしで好ペースを維持。",
      reasonFinishStronger: "ラスト1/3が最初の1/3より {delta}% 速い。",
      reasonFinishFade: "ラスト1/3が最初の1/3より {delta}% 遅い。",
      reasonFinishSteady: "ラスト1/3が最初の1/3から {delta}% の範囲内。",
      reasonBestRep: "レップ {rep} がセット平均より {delta} 秒/500m 速い。",
      reasonSlowestRep: "レップ {rep} がセット平均より {delta} 秒/500m 遅い。",
    },
    lowRes: "低解像度リプレイ",
    compareAgainst: "比較対象:",
    none: "なし",
    ...
    moreCompareOptions: "その他の比較オプション",
    ...
    view2dError: "2D描画エラー — ページの再読み込みで改善する場合があります",
    seekSlider: "シーク",
    playbackSpeed: "再生速度",
    pacePer500m: "500mあたりのペース",
    uploadCsvHint: "CSV、TCX、FITファイルをアップロード",
    closePanel: "閉じる",
    quality: "画質",
    qualityLow: "低",
    qualityMedium: "中",
    ...
    noSamples:
      "そのファイルに使えるワークアウトサンプルがありません。別のファイルを試すか、フォーマットを確認してください。",
    fileReadError:
      "そのファイルを読み取れませんでした。CSV、TCX、FIT エクスポートかどうか確認してください。",
    importFailed:
      "そのファイルをインポートできませんでした。有効な CSV、TCX、FIT エクスポートであることを確認してください。",
    zone1: "Z1 リカバリー",
    zone2: "Z2 エンデュランス",
    zone3: "Z3 テンポ",
    zone4: "Z4 しきい値",
  ...
  settings: {
    ...
    dataNote:
      "rowplay はアクセスのたびに Concept2 API からワークアウトをリアルタイムに読み取ります。ログイントークンは安全なブラウザ Cookie に保存され、サーバーには保存されません。ログアウトで消去されます。",
    factWorkouts: "エクスポート可能なワークアウト {n} 件",
    factDemo: "デモモード — サンプルデータのみ、永続化されません。",
    factCache:
      "ワークアウトデータは Concept2 API からリアルタイムに取得 — サーバー側キャッシュなし。",
    factSession:
      "ログイン情報は安全なブラウザ Cookie に保存されます。サーバー上にはデータはありません。",
  ...
  token: {
    ...
    introBefore: "Concept2 ログブックから個人 API トークンを貼り付け（",
    introLink: "プロフィール編集 → アプリケーション",
    introAfter:
      "）。トークンは HTTPS で送信され、検証後、安全なブラウザ Cookie にのみ保存されます — サーバーには一切保存されません。",
    trustTitle: "rowplay のトークン処理",
    trustAccessTitle: "アクセス：",
    trustAccessBody:
      "個人 Concept2 トークンはあなたとして認証します。rowplay はプロフィール、ワークアウト、ストロークデータの読み取りにのみ使用します。",
    trustStoredTitle: "保存：",
    trustStoredBody:
      "検証済みトークンは安全なブラウザ Cookie に保存され、localStorage やサーバーには保存されません。",
    trustDisconnectTitle: "切断：",
    trustDisconnectBody: "ヘッダーのログアウトでトークンとセッションが消えます。",
    trustCacheTitle: "データ：",
    trustCacheBody:
      "ワークアウトデータはリクエストごとに Concept2 API からリアルタイムに取得され、サーバー側には何も保存されません。",
    apiToken: "API トークン",
    ...
    serverUnavailable:
      "Concept2 に接続できません。サーバーが一時的に利用できない可能性があります。後でもう一度お試しください。",
    serverMisconfigured:
