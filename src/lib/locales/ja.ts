export const ja = {
  ...
  landing: {
    ...
    feat3Title: "エッジで配信",
    feat3Body: "Cloudflare から配信。Concept2 のライブデータで詳しいリプレイを表示。",
    tourEyebrow: "初回",
    tourTitle: "最初に試す 4 つ",
    tourBody:
...
いいえ — ダッシュボードとリプレイは非公開です。rowplay に公開共有やリーダーボード機能はありません。
...
まだ困っていますか？ [FAQ](/docs/faq) がさらにカバーしています。このガイドのすべてのページはヘッダーの**ヘルプ**から開けます。`,
  ...
  settings: {
    ...
    dataTitle: "保存している内容",
    dataNote:
      "rowplay は Concept2 API 経由でワークアウトを必要に応じて読み取ります。API トークンは SESSION_SECRET で httpOnly rp_tok Cookie に封印されます。セッションは暗号化された Cookie に保存され、サーバー側のデータベースは使用しません。ログアウトでセッションが消去されます。",
    factWorkouts: "エクスポート可能なワークアウト {n} 件",
    factDemo: "デモモード — サンプルデータのみ、永続化されません。",
    factCache:
      "ワークアウトデータは Concept2 API からリアルタイムに取得 — サーバー側キャッシュなし。",
    factSession:
      "セッションは暗号化された Cookie に保存され、トークンは httpOnly rp_tok に封印されます。",
  ...
  token: {
    ...
    introAfter:
      "）。ここに一度貼り付けてください — rowplay は HTTPS で Worker に送り、検証してから httpOnly rp_tok Cookie に封印し、サーバー側のログブック読み取りにのみ使用します。トークンはいかなるデータベースにも保存されません。",
    ...
    trustStoredTitle: "保存：",
    trustStoredBody:
      "検証済みトークンは httpOnly rp_tok Cookie に封印され、localStorage やサーバー側のストレージには保存されません。",
    trustDisconnectTitle: "切断：",
    trustDisconnectBody: "ヘッダーのログアウトでトークン Cookie とセッションが消えます。",
    trustCacheTitle: "キャッシュ：",
    trustCacheBody:
      "ワークアウトデータは Concept2 API からリアルタイムに取得されます。rowplay は公開共有やリーダーボード項目を作成しません。",
    apiToken: "API トークン",
