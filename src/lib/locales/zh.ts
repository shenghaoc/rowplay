...
2D 视图继续绘制轻量的程序化运动员形象。3D 视图可以加载专为 rowplay 制作、由仓库拥有的紧凑 low-poly 运动员与项目器械几何包，并把这些经过设计的外壳适配到同一套接触点驱动骨架上。动作与时间仍是由已记录周期驱动的确定性示意，手脚继续与器械对齐。若本地资源无法加载，程序化 3D 形象仍可使用，2D 仍是稳定的最终回退。该形象是通用示意；资源不含扫描人物、头像生成器输出、用户图像或运动员肖像。
