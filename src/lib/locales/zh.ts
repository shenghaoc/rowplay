...
rowplay 把你的室内划船、滑雪和骑行训练变成可以探索的内容：汇总与趋势的仪表板、逐桨回放、并排对比，以及轻松友好的排行榜。
...
- **对比** — 把两次训练并排放在一起，逐段比较。
- **排行榜** — 公布成绩，并与其他选手以屏幕上的「幽灵」形式竞速。

## 指南章节

- [快速上手](/docs/getting-started) — 演示模式、连接日志、首次同步。
...
- [常用操作](/docs/workflows) — 回放、幽灵竞速、对比、分享、导出。
...
4. 粘贴令牌并提交。
5. 在仪表板上点击**同步**，载入你的训练历史。

令牌只通过加密连接发送一次，并且只保存在受保护的浏览器 Cookie 中。rowplay 的服务器会缓存训练数据让页面加载更快，但绝不会保存令牌本身。

## 首次同步

首次同步会立即载入最近的训练，并在后台继续补全更早的历史。在它完成之前，长期汇总和个人最佳可能看起来不完整 — 这是正常的。如果之后仍有问题，请看[故障排查](/docs/troubleshooting)。

## 断开连接

随时可以打开[数据](/settings)断开连接。这会清除你的会话，并从 rowplay 删除缓存的训练数据。你的 Concept2 日志永远不会被修改。`,
      },
      rowingMetrics: {
        navTitle: "划船基础",
        markdown: `# 划船基础
...
PB 面板跟踪你在标准距离（500m、1k、2k、5k、6k、10k 等）上的最快成绩。在相信历史最佳之前，请确认完整同步已经完成 — 见[故障排查](/docs/troubleshooting)。
...
运动员动画与训练的真实桨频同步——每记录一桨，人物就划一桨（或撑一次杖、蹬一圈踏板），入水瞬间伴随水花或雪雾，并随回放速度一起加快。3D 中的人物改为更接近真人比例的分段身体，有躯干、头部、肩线、四肢、手脚和对应项目的运动装备，姿态读起来像正在使用测功仪的运动员，而不是玩具标记。手和脚也会贴住对应器械：划船的桨柄和脚蹬、SkiErg 的握把和靴子、BikeErg 的车把和踏板。赛道表面也会按项目变化：RowErg 有分层水道和浮标水线，SkiErg 有压过的雪道槽和蓝色门标，BikeErg 有沥青/赛车场质感、红白路缘、车道虚线和速度条。3D 追踪镜头也会保持更近的距离，让身体姿态看得见，并在艇速提高时轻微拉宽视野。

在 3D 模式下，**画质**选择器可选低、中、高、Ultra。支持 WebGPU 的设备会优先尝试更丰富的 Ultra 路径；WebGL 仍然作为回退。若设备无法保持流畅帧率，渲染器会自动先降低分辨率、再削减特效。回放动画遵循操作系统的减弱动态效果设置。

Concept2 提供逐桨数据时会优先使用。没有逐桨数据的训练会退回到分段回放，赛道仍然可以播放。

## 添加教练笔记

在回放中暂停到某个时刻并添加笔记（比如「这里滑座太急」）。笔记会钉在时间轴上，你 — 或任何收到回放分享的人 — 都能直接跳转过去。

## 与幽灵竞速

幽灵是一次过去的成绩，会在屏幕上与你并排划行。

1. 打开[排行榜](/leaderboard)，选择项目和距离。
2. 点击某条记录旁的**竞速**。
3. 你自己这段距离的回放中会出现对手，作为第二条船供你追赶。

你也可以和自己过去的成绩竞速，精确看到一次 PB 尝试在哪里赚了时间、在哪里丢了时间。

## 对比两次训练

在仪表板的训练列表中，点一次训练上的对比按钮，再选第二次。对比视图会把两次成绩逐段对齐。

## 发布到排行榜

标准距离的成绩（500m、1k、2k、5k、6k、10k、半程马拉松）可以从回放页发布到 rowplay 排行榜。发布完全自愿、可撤销，并且绝不会改动你的 Concept2 日志。

## 分享与导出

- 回放页的**分享**会生成一个公开的只读链接 — 很适合发给教练。
- [数据](/settings)页的**导出**可以把日志下载为 CSV 或 JSON；有桨数据的训练还能按次导出 TCX 文件。

## 保持数据最新

仪表板上的**同步**按需拉取新成绩。**实时模式**（也在仪表板上）会按计划轮询日志，新训练一到就通知你 — 刚练完时特别好用。

## 导入心率

如果某次训练没有心率数据但你的手表记录了，打开回放并使用**导入心率**，把手表导出的 CSV、TCX 或 FIT 文件合并进这次训练。`,
      },
      faq: {
        navTitle: "常见问题",
        markdown: `# 常见问题
...
不能 — 你的仪表板和回放默认私密。只有当你把成绩发布到排行榜，或分享了回放的公开链接，别人才能看到，而且两者都可以撤销。

## rowplay 会改动我的 Concept2 日志吗？

绝不会。rowplay 只读取。发布到 rowplay 排行榜或在这里删除缓存数据，都不会修改日志的原始记录。
...
最常见的原因是完整历史还没同步完。首次同步会在后台补全较早的训练；在它完成之前，所有「跨全部历史」计算的数据都可能不完整。到[数据](/settings)查看同步状态，必要时执行完整同步。
...
只有训练时连接了心率带或手表，日志里才有心率。如果手表单独记录了心率，请在回放页使用**导入心率**，把 CSV、TCX 或 FIT 导出文件合并进这次训练。
...
先确认训练已经到达你的 Concept2 日志（必须从器械或 ErgData 应用上传）。然后在仪表板点击**同步**，或开启实时模式自动轮询。
...
还是没解决？[常见问题](/docs/faq)涵盖更多内容，本指南的每一页都能从页眉的**帮助**进入。`,
  ...
  token: {
    ...
    introLink: "编辑资料 → 应用",
    introAfter:
      "）。在此粘贴一次 — rowplay 通过 HTTPS 发送到 Worker，验证后将其密封到 httpOnly rp_tok cookie 中，并仅用于服务器端读取日志。令牌绝不会保存在 KV 或 D1 中。",
    trustTitle: "rowplay 如何处理令牌",
    trustAccessTitle: "访问：",
    trustAccessBody:
      "个人 Concept2 令牌代表你本人认证；rowplay 只在服务器端用它读取资料、训练和逐桨数据。",
    trustStoredTitle: "存储：",
    trustStoredBody:
      "验证后的令牌会密封到 httpOnly rp_tok cookie 中，不放入 localStorage、KV 或 D1。",
    trustDisconnectTitle: "断开：",
    trustDisconnectBody: "在“数据”页退出或删除账户数据，会清除令牌 cookie、会话和私有缓存。",
    trustCacheTitle: "缓存：",
    trustCacheBody:
      "连接期间 D1 会缓存训练摘要和回放详情；只有你主动发布时才会创建公开分享或排行榜条目。",
    apiToken: "API 令牌",
    placeholder: "粘贴你的令牌",
    connect: "使用令牌连接",
    connecting: "连接中…",
