export const zh = {
  ...
  docs: {
    ...
    openDashboard: "打开仪表板",
    openSource: "打开源文件",
    guideMarkdown: `# rowplay 使用指南

rowplay 会把 Concept2 日志成绩变成仪表板、回放、对比和排行榜体验，适用于 RowErg、SkiErg 和 BikeErg。

## 从演示模式开始

演示模式是默认状态。无需登录即可打开 rowplay，应用会加载确定性的示例训练，因此你可以在没有 Concept2 账号的情况下试用仪表板和回放。

1. 打开 /dashboard。
2. 从列表中选择一次训练。
3. 点击 **Replay**，使用播放、暂停、拖动时间线和倍速控制。
4. 打开 /leaderboard，用示例数据体验幽灵竞速。

## 连接你的 Concept2 日志

生产环境使用自带令牌认证。个人 Concept2 API token 只通过 HTTPS 发送一次，由 Worker 校验，并密封进 httpOnly rp_tok cookie。KV 只保存会话身份，D1 保存缓存的训练和回放数据。token 不会写入 KV 或 D1。

1. 在 Concept2 日志中打开 **Edit Profile -> Applications**。
2. 复制个人 API token。
3. 在 rowplay 打开 /auth/token。
4. 粘贴 token 并提交。
5. 打开 /dashboard，使用 **同步** 加载完整日志历史。

可在 /settings 断开连接或删除缓存的账号数据。

## 阅读仪表板

- 使用项目和距离筛选来缩小训练列表。
- 查看总量、配速趋势、个人最佳、年度目标和训练负荷。
- 直接打开最新训练，或从列表中对比两次训练。
- 使用标签和筛选，让特定训练之后更容易找到。

如果 rowplay 只有最近历史，请先运行 **同步**，再依赖长期个人最佳或趋势。
...
- 拖动时间线检查某个具体时刻。
- 在 0.5x 到 8x 之间调整速度。
- 浏览器支持 WebGL 时，可在 2D 和 3D 渲染之间切换。
- 在训练中的某个点添加训练笔记。
- 需要在其他地方使用数据时，可导出 CSV、JSON 或回放数据。

Concept2 提供逐桨数据时会优先使用。没有逐桨数据的训练会退回到分段回放，赛道仍然可以播放。

## 幽灵竞速和训练对比

- 使用 /leaderboard 查找标准距离成绩，并启动一个对手幽灵。
- 用回放控制对比你和幽灵的配速。
- 使用 /compare 查看两次训练的并排摘要。
- 想让他人检查训练时，可以分享公开回放链接。

发布到排行榜是可选且可撤销的操作，不会改变原始 Concept2 日志条目。

## 实时模式和导入

实时模式可以在训练后轮询新训练，并在新数据出现时通知你。心率导入可以在日志条目没有心率时，把外部 HR 数据合并到训练中。

本地认证、同步、实时模式和 KV/D1 测试请使用 pnpm preview。普通 pnpm dev 做 UI 更快，但它不是 Workers 运行时，也不会提供生产 KV/D1 绑定。

## 贡献者文档政策

面向仓库的英文指南位于 docs/usage.md。应用内网站通过 locale 字典渲染指南内容，因此每个内置语言都使用同一套 i18n 路径。

当变更影响用户可见行为、工作流、路由、认证、数据处理、设置或部署预期时，请在同一个 PR 中更新 docs/usage.md、README.md 或相关仓库文档。`,
  },
  dashboard: {
    eyebrow: "你的日志",
    title: "成绩与回放",
