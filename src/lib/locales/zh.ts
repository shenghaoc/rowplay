...
不能 — 你的仪表板和回放是私密的。rowplay 没有公开分享或排行榜功能。
...
还是没解决？[常见问题](/docs/faq)涵盖更多内容，本指南的每一页都能从页眉的**帮助**进入。`,
  ...
  settings: {
    ...
    dataTitle: "我们存储的内容",
    dataNote:
      "rowplay 通过 Concept2 API 按需读取你的训练。你的 API 令牌会用 SESSION_SECRET 密封到 httpOnly rp_tok cookie 中。会话保存在加密 cookie 中，不使用服务器端数据库。退出登录会清除你的会话。",
  ...
  token: {
    ...
    trustDisconnectBody: "页眉中的“退出登录”会清除令牌 cookie 和会话。",
    trustCacheTitle: "缓存：",
    trustCacheBody: "训练数据通过 Concept2 API 实时获取；rowplay 不会创建公开分享或排行榜条目。",
    apiToken: "API 令牌",
    placeholder: "粘贴你的令牌",
    connect: "使用令牌连接",
    connecting: "连接中…",
