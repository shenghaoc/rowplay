...
本地认证、同步、实时模式和 KV/D1 测试请使用 npm run preview。普通 npm run dev 做 UI 更快，但它不是 Workers 运行时，也不会提供生产 KV/D1 绑定。
...
可用 npm run check:docs -- origin/main 在本地运行文档门禁。`
	},
	dashboard: {
		eyebrow: '你的日志',
		title: '成绩与回放',
		...
		cpScopeAll: '全部',
		cpEmptyScope:
			'可用的 {scope} 努力还不够。请先添加几个不同时长的最大努力，再信任此模型。',
		...
		cpPredictSub:
			'基于所选模型的单一运动配速和完赛时间预测。配速已标准化为 /500m。',
