...
还是没解决？[常见问题](/docs/faq)涵盖更多内容，本指南的每一页都能从页眉的**帮助**进入。`,
  ...
  settings: {
    ...
    dataTitle: "我们存储的内容",
    dataNote:
      "rowplay 通过 Concept2 API 按需读取你的训练并实时获取数据，以便即时回放。你的 API 令牌会用 SESSION_SECRET 密封到 httpOnly rp_tok cookie 中。会话保存在加密 cookie 中，不使用服务器端数据库。断开连接或删除数据会清除你的会话。",
    factWorkouts: "可导出 {n} 次训练",
    factDemo: "演示模式——仅示例数据，不会持久保存。",
    factCache: "训练数据通过 Concept2 API 实时获取 — 无服务器端缓存。",
    factSession: "会话保存在加密 cookie 中；令牌密封在 httpOnly rp_tok 中。",
    exportTitle: "导出日志",
    exportNote:
  ...
  token: {
    ...
    introLink: "编辑资料 → 应用",
    introAfter:
      "）。在此粘贴一次 — rowplay 通过 HTTPS 发送到 Worker，验证后将其密封到 httpOnly rp_tok cookie 中，并仅用于服务器端读取日志。令牌绝不会保存在任何数据库中。",
    ...
    trustStoredTitle: "存储：",
    trustStoredBody:
      "验证后的令牌会密封到 httpOnly rp_tok cookie 中，不放入 localStorage 或任何服务器端存储。",
    trustDisconnectTitle: "断开：",
    trustDisconnectBody: "在“数据”页退出或删除账户数据，会清除令牌 cookie、会话和私有缓存。",
    trustCacheTitle: "缓存：",
    trustCacheBody:
      "训练数据通过 Concept2 API 实时获取；只有你主动发布时才会创建公开分享或排行榜条目。",
    apiToken: "API 令牌",
    placeholder: "粘贴你的令牌",
    connect: "使用令牌连接",
    connecting: "连接中…",
