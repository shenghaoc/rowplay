...
运动员动画与训练的真实桨频同步——每记录一桨，人物就划一桨（或撑一次杖、蹬一圈踏板），入水瞬间伴随水花或雪雾，并随回放速度一起加快。3D 追踪镜头会在艇速提高时轻微拉宽视野。

在 3D 模式下，**画质**选择器可选低、中、高三档。若设备无法保持流畅帧率，渲染器会自动先降低分辨率、再削减特效，因此在任何硬件上都可以放心尝试高画质。回放动画遵循操作系统的减弱动态效果设置。
...
- **3D 回放无法启动** — 浏览器需要支持 WebGL；2D 视图始终可用。
