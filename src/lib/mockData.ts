...
import { nowIsoString } from "./datetime";
...
const SPECS: Spec[] = [
  ...
  {
    id: 1003,
    date: "2026-05-21 18:40:00",
    sport: "skierg",
    distance: 1000,
    basePace: 122,
    baseSpm: 42,
    baseHr: 165,
    workoutType: "1000m SkiErg",
  },
  {
    id: 1004,
  ...
  {
    id: 1005,
    date: "2026-05-16 06:20:00",
    sport: "rower",
    distance: 6000,
    basePace: 116,
    baseSpm: 28,
    baseHr: 160,
    workoutType: "4x1500m intervals",
    interval: true,
  },
  {
    id: 1006,
    date: "2026-05-13 18:15:00",
    sport: "rower",
    distance: 500,
    basePace: 96,
    baseSpm: 36,
    baseHr: 172,
    workoutType: "500m sprint",
  },
  {
    id: 1007,
    date: "2026-05-10 06:18:00",
    sport: "rower",
    distance: 2000,
    basePace: 112,
    baseSpm: 29,
    baseHr: 166,
    workoutType: "2000m steady",
  },
  {
    id: 1008,
    date: "2026-05-06 07:00:00",
    sport: "skierg",
    distance: 1000,
    basePace: 126,
    baseSpm: 40,
...
function buildSplits(spec: Spec, strokes: Stroke[]): Split[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const segTime =
      within[within.length - 1].t -
      (i === 0 ? 0 : (strokes.filter((s) => s.d <= startD).slice(-1)[0]?.t ?? 0));
...
function detailFor(spec: Spec): WorkoutDetail {
  const { strokes, time } = spec.noStrokes
    ...
    : buildStrokes(spec);
  const splits = spec.noStrokes ? [] : buildSplits(spec, strokes);
...
function summaryOf(d: WorkoutDetail): Workout {
  const { strokes: _strokes, splits: _splits, ...rest } = d;
