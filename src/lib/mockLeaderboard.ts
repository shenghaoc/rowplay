 ...
 * their metrics — no PII, no stored stroke data (a board "Race" link pre-arms a
 * constant-pace ghost from the rival's pace).
 */
...
function rivalsForBoard(sport: Sport, distance: number): LeaderboardEntry[] {
	...
	return RIVAL_NAMES.map((name, i) => {
		...
		return {
			...
			workoutId: 0, // no public replay for demo rivals
			isYou: false
