...
describe("pluralKey", () => {
  it("returns base_one for English when n = 1", () => {
    expect(pluralKey("en", "dashboard.goalsStreakCurrent", 1)).toBe(
      "dashboard.goalsStreakCurrent_one",
    );
  });
  ...
  it("returns the base key for English when n ≠ 1", () => {
    ...
    expect(pluralKey("en", "dashboard.goalsStreakCurrent", 100)).toBe(
      "dashboard.goalsStreakCurrent",
    );
  });
