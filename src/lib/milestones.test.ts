...
describe('computeMilestones — lifetime distance', () => {
	it('achieves at exact threshold and not one metre below', () => {
		...
		const notYet = computeMilestones(below, []).find((m) => m.id === 'lifetime_distance_rower_100k');
...
describe('computeMilestones — streak', () => {
	...
	it('does not reset across consecutive calendar days', () => {
		...
		const m = computeMilestones(workouts, [], { endDay: '2026-01-03' }).find((x) => x.id === 'streak_7d');
	...
	it('resets after a skipped calendar day', () => {
		...
		const m = computeMilestones(workouts, [], { endDay: '2026-01-04' }).find((x) => x.id === 'streak_7d');
	...
	it('achieves streak_7d after seven consecutive days', () => {
		...
		const m = computeMilestones(workouts, [], { endDay: '2026-01-07' }).find((x) => x.id === 'streak_7d');
		expect(m?.achieved).toBe(true);
