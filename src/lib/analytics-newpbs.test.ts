...
describe('distancePBs', () => {
	...
	it('identifies the fastest workout at each standard distance', () => {
		const w1 = workout({ id: 1, distance: 2000, time: 480, sport: 'rower', date: '2026-05-01 06:00:00' });
		const w2 = workout({ id: 2, distance: 2000, time: 490, sport: 'rower', date: '2026-04-01 06:00:00' });
	...
	it('returns separate PBs per sport', () => {
		const rower = workout({ id: 1, distance: 2000, time: 480, sport: 'rower', date: '2026-05-01 06:00:00' });
		const bike = workout({ id: 2, distance: 2000, time: 450, sport: 'bike', date: '2026-05-01 06:00:00' });
	...
	it('applies ±2% tolerance to distance matching', () => {
		...
		const w = workout({ id: 1, distance: 2020, time: 480, sport: 'rower', date: '2026-05-01 06:00:00' });
		const pbs = distancePBs([w]);
		expect(pbs.find((p) => p.distance === 2000)).toBeDefined();
	});

	it('ignores workouts with time = 0', () => {
		const w = workout({ id: 1, distance: 2000, time: 0, sport: 'rower', date: '2026-05-01 06:00:00' });
