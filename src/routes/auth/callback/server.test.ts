...
vi.mock('$lib/server/config', () => ({
	getConfig: vi.fn().mockReturnValue({ clientId: 'cid', clientSecret: 'csecret', redirectUri: 'http://localhost/auth/callback' }),
	requireSessions: vi.fn().mockReturnValue({ put: vi.fn(), get: vi.fn(), delete: vi.fn() })
}));
vi.mock('$lib/server/concept2', () => ({
	exchangeCode: vi.fn().mockResolvedValue({ accessToken: 'tok', refreshToken: 'rtok', expiresAt: 9999999, scope: 'read' }),
