...
function fakeGetEvent(year?: string) {
	const url = year
		? `http://localhost/api/goals?year=${year}`
...
describe('PUT /api/goals', () => {
	it('throws 400 when kind is invalid', async () => {
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		await expect(PUT(fakePutEvent({ kind: 'miles', target: 1000 }) as any)).rejects.toMatchObject({ status: 400 });
	});

	it('throws 400 when target is missing', async () => {
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		await expect(PUT(fakePutEvent({ kind: 'meters' }) as any)).rejects.toMatchObject({ status: 400 });
	});

	it('throws 400 when target is zero or negative', async () => {
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		await expect(PUT(fakePutEvent({ kind: 'meters', target: 0 }) as any)).rejects.toMatchObject({ status: 400 });
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		await expect(PUT(fakePutEvent({ kind: 'hours', target: -1 }) as any)).rejects.toMatchObject({ status: 400 });
	});
