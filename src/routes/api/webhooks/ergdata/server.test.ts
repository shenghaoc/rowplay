...
function fakeEvent(opts: { secret?: string; sig?: string | null; body?: string }) {
  ...
  return {
    ...
    request: {
      headers: {
        get: (name: string) =>
          name.toLowerCase() === "x-ergdata-signature" ? (opts.sig ?? null) : null,
      },
      text: async () => body,
    },
  };
...
describe("POST /api/webhooks/ergdata", () => {
  ...
  it("throws 401 when signature is missing", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await expect(POST(fakeEvent({ secret: "mysecret", sig: null }) as any)).rejects.toMatchObject({
      status: 401,
    });
  });
  ...
  it("throws 401 when signature is invalid", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await expect(
      POST(fakeEvent({ secret: "mysecret", sig: "sha256=" + "0".repeat(64) }) as any),
  ...
  it("throws 401 for wrong-length signature hex", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await expect(
      POST(fakeEvent({ secret: "mysecret", sig: "sha256=tooshort" }) as any),
    ).rejects.toMatchObject({ status: 401 });
  });
  ...
  it("throws 400 when JSON is invalid", async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await expect(POST(fakeEvent({ secret, sig, body }) as any)).rejects.toMatchObject({
      status: 400,
    });
  });
