...
describe("POST /api/workouts/[id]/annotations", () => {
  it("throws 400 for non-numeric id", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await expect(
      POST(fakePostEvent("abc", { timestamp: 0, text: "hi" }) as any),
    ).rejects.toMatchObject({ status: 400 });
  });

  it("throws 400 when timestamp is missing", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await expect(POST(fakePostEvent("1001", { text: "hi" }) as any)).rejects.toMatchObject({
      status: 400,
    });
  });
  ...
  it("throws 400 when timestamp is negative", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await expect(
      POST(fakePostEvent("1001", { timestamp: -1, text: "hi" }) as any),
    ).rejects.toMatchObject({ status: 400 });
  });
  ...
  it("throws 400 when text is empty", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await expect(
      POST(fakePostEvent("1001", { timestamp: 0, text: "   " }) as any),
    ).rejects.toMatchObject({ status: 400 });
  });
  ...
  it("throws 400 when text exceeds 1000 chars", async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await expect(
      POST(fakePostEvent("1001", { timestamp: 0, text: longText }) as any),
    ).rejects.toMatchObject({ status: 400 });
  });
  ...
  it("throws 400 when annotation id is non-integer", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await expect(
      POST(fakePostEvent("1001", { id: 1.5, timestamp: 0, text: "hi" }) as any),
    ).rejects.toMatchObject({ status: 400 });
  });
