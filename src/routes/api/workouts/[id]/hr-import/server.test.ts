...
describe('POST /api/workouts/[id]/hr-import', () => {
	it('throws 400 for non-numeric workout id', async () => {
		...
		await expect(POST(fakePostEvent('abc', { samples: validSamples, offset: 0 }) as any)).rejects.toMatchObject({ status: 400 });
	});

	it('throws 400 when body is missing samples array', async () => {
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		await expect(POST(fakePostEvent('1001', { offset: 0 }) as any)).rejects.toMatchObject({ status: 400 });
	});

	it('throws 400 when a sample has invalid fields', async () => {
		const badSamples = [{ t: 'notanumber', hr: 140 }];
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		await expect(POST(fakePostEvent('1001', { samples: badSamples, offset: 0 }) as any)).rejects.toMatchObject({ status: 400 });
	});
