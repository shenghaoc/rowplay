...
movement physics except for the later, explicitly documented RowErg technique
and shell-orientation correction.
...
4. **Motion**
   - clip names, drive ends, contact offsets, and IK solve remain authoritative;
     the rowing follow-up retimes only the existing leg/body/arm overlap windows
     to establish leg-led drive and hands/body/slide recovery, while a
     microscopic BikeErg saddle-compatibility relief remains visual-only
