...
test.describe("smoke", () => {
  test("language picker switches dashboard copy", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Ergebnisse & Replays/ })).toBeVisible();
  ...
  test("dashboard loads cleanly and lists workouts", async ({ page }) => {
    ...
    await expect(page.getByText(/Season goals|赛季目标/)).toBeVisible();
  ...
  test("dashboard hierarchy keeps workouts before advanced analysis and hints persist dismissal", async ({
    page,
  }) => {
    const errors = collectPageErrors(page);
    ...
    const order = await page
      .locator(
        '[data-e2e="core-summary"], [data-e2e="workout-section"], [data-e2e="advanced-section"]',
      )
      .evaluateAll((els) =>
        els.map((el) => {
  ...
  test("workout list filters update the URL and narrow results", async ({ page }) => {
    ...
    await page
      .getByRole("group", { name: /Sort|排序/ })
      .getByRole("button", { name: /Pace|配速/ })
      .click();
  ...
  test("service worker caches viewed replay for offline", async ({ page }) => {
    ...
    expect(cached, `replay SSR page should be cached; got ${JSON.stringify(cacheDump)}`).toBe(true);
  ...
  test("settings page loads with export controls", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Account & data|账户与数据/ })).toBeVisible();
  ...
  test("token entry page renders its form", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /How rowplay handles the token/i }),
    ).toBeVisible();
