...
- Viewports: 1440×1024 browser / 1140×420 replay stage and 390×844
  browser / 390×360 replay stage
- Captured states: 2D desktop dark; 3D desktop light/dark Ultra; 3D mobile
  light Low; representative paused/moving and live/ghost comparison states
...
No actionable P0, P1, or P2 **visual implementation** findings remain. One
release-evidence finding remains open while the pull request is a draft.

- [P1] The captured matrix does not yet prove every permutation named by the
  authored-asset specification.
  Location: release evidence, not the renderer implementation.
  Evidence: Canvas light/mobile, 3D mobile dark, HUD-hidden grayscale/dark
  silhouette, and an operating-system reduced-motion frame are absent. The
  reduced-motion and fallback branches are mechanically covered by tests, but
  tests do not substitute for the missing frames.
  Impact: the visual implementation is accepted from the captured states, but
  the draft is not ready to claim the specification's full release matrix.
  Follow-up: capture those states in an approved browser session before moving
  the PR out of draft.
...
- Spacing and layout rhythm: the 1140×420 and 390×360 stages retain their real
  aspect, controls remain outside the canvas, and no stage or persistent control
  overlaps at either breakpoint.
...
- Observed no error notification, blank replay, or renderer fallback during the
  browser passes. Loader/backend failure and reduced-motion branches are covered
  by focused tests because the selected browser session exposes WebGPU and its
  media preference cannot be mutated by the read-only browser QA surface.
...
- [ ] Complete Canvas light/mobile, 3D mobile dark, HUD-hidden silhouette, and
...
final result: visual pass; release-evidence pass pending
