...
- [ ] **1. Window constant + pure helpers** — `src/lib/server/historyWindow.ts`
...
- [ ] **2. Unit tests for the pure core** — `historyWindow.test.ts`
...
- [ ] **3. D1 watermark** — `migrations/0006_history_window.sql` + `db.ts`
...
- [ ] **6. Backfill run + endpoint** — `data.ts` + `src/routes/api/sync/backfill/+server.ts`
...
- [ ] **7. Background driver + on-demand trigger** — dashboard/settings
...
- [ ] **8. Sync-status affordance** — dashboard + settings + `syncStatus`
...
- [ ] **10. E2E + full gate** — `tests/e2e/history-windowing.spec.ts`
