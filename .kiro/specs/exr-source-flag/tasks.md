...
- [ ] 1. Pure detector + unit tests
  - [ ] 1.1 Create `src/lib/exrSource.ts` exporting `isExrSource(workout: Pick<Workout, 'source'>): boolean`.
        The function returns `true` iff `workout.source?.toUpperCase() === 'EXR'`.
        No DOM access; safe in server, client, and test contexts.
  - [ ] 1.2 Create `src/lib/exrSource.test.ts` with Vitest unit tests covering:
        - `source: 'EXR'` → `true`
        - `source: 'exr'` (lowercase) → `true`
        - `source: 'ErgData'` → `false`
        - `source: 'Web'` → `false`
        - `source` absent (`{}`) → `false`
  - [ ] 1.3 Run `npm run test` locally to confirm the new tests pass before
        proceeding.
  - _Requirements: 1.1, 1.2, 1.3, 1.4, 1.5_

- [ ] 2. i18n keys — all six locales
  - [ ] 2.1 Add `replay.mSource`, `replay.exrBadge`, and `replay.exrBadgeTitle`
        ...
        for readability.
  - [ ] 2.2 Add the same three keys (with appropriate translations or placeholders)
        to `zh.ts`, `de.ts`, `es.ts`, `fr.ts`, and `ja.ts`.
  - [ ] 2.3 Run `npm run validate:locales` and confirm zero missing-key errors.
  - _Requirements: 6.1, 6.2, 6.3_

- [ ] 3. Demo mode — EXR-sourced mock workout
  - [ ] 3.1 Add optional `source?: string` to the `Spec` interface in
        `src/lib/mockData.ts`.
  - [ ] 3.2 Update `detailFor()` to set `detail.source = spec.source` when
        `spec.source` is present (one-liner alongside existing field assignments).
  - [ ] 3.3 Add `source: 'EXR'` to the `id: 1004` "8000m BikeErg" entry in
        `SPECS` (or a different existing entry if preferred).
  - [ ] 3.4 Manually verify `/replay/1004` in demo mode shows the EXR badge
        (after badge rendering is added in Task 4).
  - _Requirements: 5.1, 5.2, 5.3, 5.4_

- [ ] 4. EXR badge on the replay / workout-detail page
  - [ ] 4.1 Import `isExrSource` from `$lib/exrSource` in
        `src/routes/replay/[id]/+page.svelte`.
  - [ ] 4.2 Add a `$derived` reactive variable:
        `const exrFlagged = $derived(isExrSource(detail));`
  - [ ] 4.3 In the `.summary` row (after the `lowRes` badge), add:
        ```svelte
        {#if exrFlagged}
          <span class="badge" title={t('replay.exrBadgeTitle')}>{t('replay.exrBadge')}</span>
        {/if}
        ```
  - [ ] 4.4 In the "Logging provenance" `<dl>` block (currently after
...
- [ ] 5. EXR badge on the public share page
  - [ ] 5.1 Import `isExrSource` from `$lib/exrSource` in
        `src/routes/r/[token]/+page.svelte`.
  - [ ] 5.2 Add the same `exrFlagged` derived variable and the badge in the
        header area of the share page, using the same `replay.exrBadge` and
        `replay.exrBadgeTitle` keys.
  - [ ] 5.3 Confirm `redactForPublic()` in `src/lib/server/share.ts` requires
        no changes (the `source` field is not in the strip list).
  - _Requirements: 4.1, 4.2, 4.3_

- [ ] 6. Quality gate
  - [ ] 6.1 `npm run check` → zero type errors.
  - [ ] 6.2 `npm run build` → succeeds.
  - [ ] 6.3 `npm run test` → green (includes the new `exrSource.test.ts`).
  - [ ] 6.4 Manual demo verification:
