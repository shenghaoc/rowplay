...
`static/replay-assets/rowplay-athlete-v4.glb` is generated by
`scripts/build-replay-rig-v4.mjs`. It round-trips as one indexed
`SkinnedMesh`/primitive with vertex-colour skin, fabric, shorts, trim, hair,
headwear, and footwear regions; its three named one-second clips carry the
reviewed drive boundaries and phase schema in glTF extras. Palm/sole contact
offsets are stored on terminal bones and validated from the checked binary.
The `rigV4.ts` authoring module and `rowplay-athlete-v4.glb` are production
contracts.
